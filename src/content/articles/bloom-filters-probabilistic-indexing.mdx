...
```mermaid
graph TD
    KEY[Query Key] -- Apply k Hash Functions --> HASHES["Compute Bit Indices h1(x), h2(x)... hk(x)"]
    HASHES -- Inspect Bit Array --> COND{Are ALL bits set to 1?}
    COND -- No (At least one bit is 0) --> NO["DEFINITELY NOT IN SET (100% Certainty)"]
    COND -- Yes (All bits are 1) --> MAYBE["MAYBE IN SET (Execute Disk/DB Index Lookup)"]
```

---

## 4. Probabilistic vs Exact Search Structures
